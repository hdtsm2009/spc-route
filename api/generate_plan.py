"""Vercel Serverless Function: generate_plan
POST /api/generate_plan

Netlify版からの移植。ハンドラ形式のみ変更（BaseHTTPRequestHandler）。
ロジック本体は共通。
...
def _load_data():
    global _STORES, _CONFIG
    if _STORES is None:
        with open(os.path.join(_DIR, "stores.json"), encoding="utf-8") as f:
            _STORES = json.load(f)
...
def _fill_block(start_min, end_min, cur_lat, cur_lng,
    ...
    budget = route_cfg.get("_budget", 99)
    plan = []
    cur = start_min
    for r in pool:
        if len(plan) >= budget:
